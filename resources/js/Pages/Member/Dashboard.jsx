...
import { 
    BookOpen, Trophy, Sparkles, PlayCircle, Clock, 
    ChevronRight, Compass, Star, MessageSquare, X 
} from 'lucide-react';
...
{/* ======================================= */}
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 sm:gap-8">
                    ...
                    <div className="flex-1 min-w-0 w-full">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/10 backdrop-blur-md border border-white/10 text-blue-200 text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-5 sm:mb-6">
                            <Sparkles size={14} className="text-blue-300 shrink-0" /> Ruang Belajar Utama
                        </div>
                        
                        {/* Tambahan leading-normal dan pb-2 untuk mencegah harakat Arab (seperti fathatain/dhammahtain) terpotong */}
                        <h1 className="text-4xl sm:text-5xl lg:text-6xl text-white font-arabic mb-2 drop-shadow-lg leading-normal pb-2" dir="rtl">
                            أَهْلًا وَسَهْلًا
                        </h1>
    ...
    Semoga Allah memberikan kemudahan dalam setiap langkah belajarmu. Lanjutkan progres materi hari ini dan persiapkan dirimu dengan <span className="inline-block whitespace-nowrap">sebaik-baiknya.</span>
</p>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-[1.5rem] p-6 shadow-sm border border-slate-100 flex items-center gap-5 group hover:shadow-xl hover:shadow-blue-950/5 transition-all duration-300">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-[1.5rem] p-6 shadow-sm border border-slate-100 flex items-center gap-5 group hover:shadow-xl hover:shadow-emerald-950/5 transition-all duration-300">
            ...
            <div className="mb-6 flex items-center justify-between">
                ...
                <Link href={route('member.courses.index')} className="hidden sm:flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                {recentCourses.length === 0 ? (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full bg-white rounded-[2rem] border border-dashed border-slate-200 p-12 text-center flex flex-col items-center">
                        ...
                        <Link href={route('member.catalog')} className="px-6 py-3 bg-blue-950 text-white font-bold rounded-xl">Eksplorasi Katalog</Link>
                    </motion.div>
                ) : (
                    recentCourses.map((course, index) => (
                        <motion.div key={course.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + (index * 0.1) }} className="bg-white rounded-[1.5rem] p-3 shadow-sm border border-slate-100 flex flex-col hover:shadow-2xl transition-all group">
                            <div className="h-40 bg-slate-100 rounded-[1.25rem] relative overflow-hidden mb-4">
                                {course.thumbnail_url ? <img src={course.thumbnail_url} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-slate-300"><BookOpen size={40} /></div>}
                            </div>
                            <h4 className="text-base font-black text-slate-800 px-2 line-clamp-2 mb-4">{course.nama}</h4>
                            <Link href={route('member.courses.show', course.id)} className="mt-auto w-full py-2.5 bg-slate-50 text-blue-950 border border-slate-200 rounded-xl text-sm font-bold hover:bg-blue-950 hover:text-white transition-all text-center">Lanjutkan</Link>
                        </motion.div>
                    ))
                )}
            </div>
                ...
                {!isReviewOpen ? (
                    ...
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h4 className="font-bold text-slate-900">Puas Belajar di MiqotHub?</h4>
                                <p className="text-sm text-slate-500 font-medium">Bantu kami berkembang dengan memberikan testimoni Anda.</p>
                            </div>
                        </div>
                        <button 
                    ...
                    >
                        <button onClick={() => setIsReviewOpen(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600 transition-colors">
                            <X size={24} />
                        </button>
                        ...
                        <div className="flex flex-col md:flex-row gap-8 items-start">
                            <div className="md:w-1/3">
                                ...
                                <h3 className="text-2xl font-black text-slate-900 mb-3 tracking-tight">Bagikan Pengalaman Belajarmu!</h3>
                                <p className="text-slate-500 text-sm font-medium leading-relaxed">
                                    Ulasanmu akan ditampilkan di landing page untuk membantu calon member lainnya.
                                </p>
                            </div>
                            ...
                            <div className="flex-1 w-full bg-slate-50 p-6 md:p-8 rounded-[1.5rem] border border-slate-100">
                                <form onSubmit={submitReview} className="space-y-6">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Rating Bintang</label>
                                        <div className="flex gap-2">
                                            {[1, 2, 3, 4, 5].map((star) => (
                                                <button key={star} type="button" onClick={() => setData('rating', star)} className="transition-transform active:scale-90">
                                                    <Star size={32} className={data.rating >= star ? 'text-amber-400 fill-amber-400' : 'text-slate-300'} strokeWidth={2} />
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Tulis Kesan Pesan</label>
                                        <textarea 
                                            value={data.komentar}
                                            ...
                                            required
                                        ></textarea>
                                        {errors.komentar && <p className="text-rose-500 text-xs mt-2 font-semibold">{errors.komentar}</p>}
                                    </div>

                                    <div className="flex gap-3">
                                        <button type="submit" disabled={processing} className="px-10 py-3.5 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all disabled:opacity-50">
                                            {processing ? 'Mengirim...' : 'Kirim Testimoni'}
                                        </button>
                                        <button type="button" onClick={() => setIsReviewOpen(false)} className="px-6 py-3.5 text-slate-400 font-bold text-sm hover:text-slate-600">Batal</button>
                                    </div>
                                </form>
                            </div>
                        </div>
